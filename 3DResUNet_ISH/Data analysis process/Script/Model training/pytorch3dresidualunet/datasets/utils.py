...
class SliceBuilder:
    ...
    def __init__(self, raw_dataset, label_dataset,CA1_dataset,CA2_dataset,CA3_dataset,DG_dataset,MASK_dataset, weight_dataset, patch_shape, stride_shape, **kwargs):#
        """
        ...
        """
        # patch_shape[1], patch_shape[2] = patch_shape[2], patch_shape[1]
        ...
        if label_dataset is None:
            ...
            # self._raw_slices = None
            self._CA1_slices = None
            self._CA2_slices = None
            self._CA3_slices = None
            self._DG_slices = None
            self._MASK_slices = None
        else:
            ...
            self._CA1_slices = self._build_slices(CA1_dataset, patch_shape, stride_shape)
            self._CA2_slices = self._build_slices(CA2_dataset, patch_shape, stride_shape)
            self._CA3_slices = self._build_slices(CA3_dataset, patch_shape, stride_shape)
            self._DG_slices = self._build_slices(DG_dataset, patch_shape, stride_shape)
            #self._mask_slices = self._build_slices(mask_dataset, patch_shape, stride_shape)
            self._MASK_slices = self._build_slices(MASK_dataset, patch_shape, stride_shape)
    ...
    @property
    def CA1_slices(self):
        return self._CA1_slices

    @property
    def CA2_slices(self):
        return self._CA2_slices

    @property
    def CA3_slices(self):
        return self._CA3_slices

    @property
    def DG_slices(self):
        return self._DG_slices

    # @property
    # def mask_slices(self):
    #     return self._mask_slices
    ...
    def _check_patch_shape(patch_shape):
        ...
        assert patch_shape[1] >= 8 and patch_shape[2] >= 8, 'Height and Width must be greater or equal 64'
...
class FilterSliceBuilder(SliceBuilder):
    ...
    def __init__(self, raw_dataset, label_dataset,CA1_dataset,CA2_dataset,CA3_dataset,DG_dataset,MASK_dataset, weight_dataset, patch_shape, stride_shape, ignore_index=(0,),
                 threshold=0.6, slack_acceptance=0.01, **kwargs):#
        super().__init__(raw_dataset,label_dataset,CA1_dataset,CA2_dataset,CA3_dataset,DG_dataset, MASK_dataset,weight_dataset, patch_shape, stride_shape, **kwargs)#
        if label_dataset is None:
            return
        ...
        zipped_slices = zip(self.raw_slices, self.label_slices,self.CA1_slices,self.CA2_slices,self.CA3_slices,self.DG_slices_slices,self.MASK_slices)#
        ...
        raw_slices,label_slices ,CA1_slices,CA2_slices,CA3_slices,DG_slices,MASK_slices= zip(*filtered_slices)#
        self._raw_slices = list(raw_slices)
        self._label_slices = list(label_slices)
        self._CA1_slices = list(CA1_slices)
        self._CA2_slices = list(CA2_slices)
        self._CA3_slices = list(CA3_slices)
        self._DG_slices = list(DG_slices)
        # self._mask_slices = list(mask_slices)
        self._MASK_slices = list(MASK_slices)
...
def get_slice_builder(raws,labels,CA1s ,CA2s ,CA3s ,DGs ,MASKs,weight_maps, config):#
    assert 'name' in config
    logger.info(f"Slice builder config: {config}")
    slice_builder_cls = _loader_classes(config['name'])
    return slice_builder_cls(raws,labels,CA1s ,CA2s ,CA3s ,DGs,MASKs, weight_maps, **config) #
...
def get_train_loaders(config):
    ...
    # CA1_datasets = dataset_class.create_datasets(loaders_config, phase='CA1')
    #
    # CA2_datasets = dataset_class.create_datasets(loaders_config, phase='CA2')
    #
    # CA3_datasets = dataset_class.create_datasets(loaders_config, phase='CA3')
    #
    # DG_datasets = dataset_class.create_datasets(loaders_config, phase='DG')
    #
    # mask_datasets = dataset_class.create_datasets(loaders_config, phase='mask')
    ...
    return {
        ...
        # 'CA1': DataLoader(ConcatDataset(CA1_datasets), batch_size=batch_size, shuffle=False, num_workers=num_workers),
        # 'CA2': DataLoader(ConcatDataset(CA2_datasets), batch_size=batch_size, shuffle=False, num_workers=num_workers),
        # 'CA3': DataLoader(ConcatDataset(CA3_datasets), batch_size=batch_size, shuffle=False, num_workers=num_workers),
        # 'DG': DataLoader(ConcatDataset(DG_datasets), batch_size=batch_size, shuffle=False, num_workers=num_workers),
        # 'mask': DataLoader(ConcatDataset(mask_datasets), batch_size=batch_size, shuffle=False, num_workers=num_workers)
    }
