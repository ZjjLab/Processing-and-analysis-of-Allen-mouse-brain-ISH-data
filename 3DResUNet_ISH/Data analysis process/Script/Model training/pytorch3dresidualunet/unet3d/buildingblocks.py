...
def depth_conv(in_channels, out_channels, kernel_size, bias, padding):
    return nn.Conv3d(in_channels, in_channels, kernel_size, stride=1, padding=padding, groups=in_channels)

def point_conv(in_channels, out_channels, kernel_size, bias, padding):
    return nn.Conv3d(in_channels, out_channels, 1, stride=1, padding=0, groups=1)
...
def create_conv(in_channels, out_channels, kernel_size, order, num_groups, padding):
    ...
    for i, char in enumerate(order):
        if char == 'r':
            ...
            # modules.append(('depth_conv', depth_conv(in_channels, out_channels, kernel_size, bias, padding=padding)))
            # modules.append(('point_conv', point_conv(in_channels, out_channels, kernel_size, bias, padding=padding)))
        elif char == 'g':
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, apply_pooling=True,
        ...
        self.basic_module = basic_module(in_channels, out_channels,
                                         ...
                                         padding=padding)
        # 添加残差连接，1*1卷积改变通道数
        self.conv = nn.Conv3d(in_channels, out_channels, kernel_size=1, padding=(0, 0, 0), bias=False)
...
def create_decoders(f_maps, basic_module, conv_kernel_size, conv_padding, layer_order, num_groups, upsample):
    ...
    decoders = []
    reversed_f_maps = list(reversed(f_maps))    # [64, 128, 256, 512]
