...
class DefaultTensorboardFormatter(_TensorboardFormatter):
    ...
    def _normalize_img(img):
        return np.nan_to_num((img - np.min(img)) / (np.ptp(img)+1e-12))
...
def create_optimizer(optimizer_config, model):
    ...
    # centerloss = CenterLoss(num_classes=4, feat_dim=4, use_gpu=torch.cuda.is_available())
    # optimizer_CL = optim.SGD(centerloss.parameters(), lr=0.5)
    return optimizer
