...
class StandardPredictor(_AbstractPredictor):
    ...
    def __call__(self, test_loader):
        ...
        with torch.no_grad():
            for batch, indices in test_loader:
                ...
                predictions[batch>0] = batch[batch>0]
        ...
        # prediction_map = predictions.squeeze().permute(2,0,1).cpu().numpy()
        prediction_map = predictions.squeeze().cpu().numpy()
        h5_output_file.create_dataset(prediction_datasets[0], data=prediction_map, compression="gzip")
    ...
    def _allocate_prediction_maps(self, output_shape, output_heads, output_file):
        ...
        normalization_masks = [np.ones(output_shape, dtype='uint8') for _ in range(output_heads)]
    ...
    def _save_results(self, prediction_maps, normalization_masks, output_heads, output_file, dataset):
        ...
        for prediction_map, normalization_mask, prediction_dataset in zip(prediction_maps, normalization_masks,
                                                                          prediction_datasets):
            prediction_map = prediction_map / (normalization_mask+1)
            ...
            prediction_map = prediction_map.squeeze(0).transpose(2, 0, 1)
